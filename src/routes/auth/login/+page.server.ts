...
export const actions: Actions = {
	login: async (event) => {
		...
		if (!validatePassword(password)) {
			return fail(400, {
				message: 'Kata sandi tidak valid (min 6, max 255 karakter)',
				action: 'login'
			});
		}
		...
		if (results.length === 0) {
			...
			const normalizedIdentity =
				typeof identity === 'string' && /^\d+$/.test(identity) && identity.startsWith('0')
					? '62' + identity.slice(1)
					: identity;
			results = await db
				.select()
				.from(table.user)
				.where(eq(table.user.contactNumber, normalizedIdentity));
		}
	...
	register: async (event) => {
		...
		if (!validateFullName(fullName)) {
			return fail(400, {
				message: 'Nama lengkap tidak valid (min 2, max 100 karakter)',
				action: 'register'
			});
		}
		...
		if (!validateContactNumber(contactNumber)) {
			return fail(400, {
				message: 'Nomor kontak tidak valid (min 10, max 15 digit)',
				action: 'register'
			});
		}
		if (!validatePassword(password)) {
		...
		try {
			...
			while (true) {
				const existing = await db
					.select()
					.from(table.user)
					.where(eq(table.user.username, finalUsername));
...
function validateIdentity(identity: unknown): identity is string {
	return typeof identity === 'string' && identity.length >= 3 && identity.length <= 31;
}
